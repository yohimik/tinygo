...
// so it is parked with task.Pause and resumed from the scheduler's idle hook
// (waitForEvents, which calls checkSignals below). The signal handler itself
// only has to record the signal and exit the futex wait the idle hook is
// sitting in.
