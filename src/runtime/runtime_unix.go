...
func tinygo_signal_handler(s int32) {
	...
	// Wake the goroutine inside os/signal that hands signals to the channels
	// registered with signal.Notify. With the cooperative scheduler that
	// goroutine is resumed by checkSignals below; with threads it is a real
	// thread blocked on a futex of its own, which this wakes.
	// Everything this does is a store to an atomic and a futex wake syscall,
	// both of which are safe to call from a signal handler.
	signalRecvWake()
}
